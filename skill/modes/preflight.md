...
- §8.4. `Mandatory read sets`; §8.5. `Cold resume check`; §8.7. `Stale checkpoint guard`; §8.8. `` `0G` invocation log ``
...
Validate version/mode, namespace and IDs, explicit persona slugs, canonical `<persona-prefix>-<persona-slug>` basenames, registry/header prefix agreement, prefix uniqueness/non-reuse, `_shared` exclusion, canonical entry ownership, concrete track/matrix structure, and absence of wildcard-only closure.
