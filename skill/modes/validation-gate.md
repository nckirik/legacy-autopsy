...
Deterministic `21`/`22` or exact stage reports with stable registry Check IDs, complete evidence bindings/input fingerprints, and append-only `0G`. Final verification may additionally emit only the §15.5 non-authoritative receipt outside the package and protocol state.

## Forbidden mutations

No source, forensic, synthesis, handbook, decision, or confirmation semantics; no hand-edited arithmetic, unsupported pass, pre-step-6 Passed state, authoritative post-step-6 completion report, or receipt treated as package/protocol authority.

## Deterministic validation

Run applicable structural Markdown, enum, ID, hash/envelope, reference, arithmetic, exact stage check-registry/evidence-set, schema/order, handbook, package-member/snapshot/signature, §15.1, §15.5 final-bundle, and conformance-fixture checks; report unsupported checks explicitly.

## Completion condition

The named report deterministically records the exact registry set, result, offending IDs, evidence/input fingerprints, validator version, and stage state. Exit E passes only after §15.5 validation of the completed signed step-6 outer manifest; the optional receipt is non-authoritative and outside the package/protocol state.
