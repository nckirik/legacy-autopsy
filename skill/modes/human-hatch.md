...
Load subtype prerequisites: escalation ticket/static investigation, closed escalation reason, and reason-consistent probe feasibility; decision inputs, exact COV arithmetic, alternatives/risks; or passing candidate/manifest, semantic and DEC bindings, dependency closure, complete candidate HBK set, readability review result/limitations, and reviewer authority for confirmation.
...
Validate subtype/action and authority, closed escalation reason plus runtime/non-runtime §9.3 prerequisites, DEC/CNF hashes and envelope bindings, denominator arithmetic, candidate/closure identity, complete passing HBK readability-review coverage where applicable, signatures/timestamps, and no semantic-byte change for envelope-only actions.
