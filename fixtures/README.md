...
M0 cases in `positive/foundations.json` and `negative/foundations.json` cover protocol discovery, foundational IDs/path normalization, prefix-qualified persona-directory basename parsing, explicitly limited basic Markdown hashing, and workspace-skeleton structure. The persona-directory slice validates only the §2.1 basename grammar and reserved `_shared` rejection; complete registry agreement and ownership remain in the unsupported Part 19.2 family. CI runs all registered cases and the CLI lists every complete §19.2 family as unsupported. See [docs/conformance.md](../docs/conformance.md) and [docs/roadmap.md](../docs/roadmap.md).
